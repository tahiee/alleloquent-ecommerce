    ...
    if (typeof window === 'undefined') {
      try {
        ...
        if (!projectId || !privateKey || !clientEmail) {
          console.warn('Firebase Admin credentials not configured. Some server-side features may not work.')
          console.warn('Please check your .env.local file and ensure these variables are set:')
        ...
        if (privateKey.includes('ABC12345') || privateKey.length < 100) {
          console.error('Firebase Admin private key appears to be a placeholder!')
          console.error('Please download the real service account key from Firebase Console')
